...
out()
out("- **Whale Safe's public API does not carry whale data.** Checked against its")
out("  OpenAPI spec: all nine endpoints are ship-side - operator scorecards,")
out("  vessel-speed-reduction compliance grades, AIS track segments. The near-real-time")
out("  whale-presence rating (acoustic detections + sightings + habitat model) reaches")
out("  the shipping industry privately and everyone else through the website map. The")
out("  only route to it as data is a request to boi-whalesafe@ucsb.edu. The VSR data is")
out("  not a substitute: speed-reduction seasons are declared in advance, so they encode")
out("  an expectation of whales rather than an observation of them.")
